...
**Spool tags** (NTAG213, URL only in Phase 1):

```text
http://{MOMENT_HOST}/nfc/spool/{spoolman-spool-id}
```

**Location tags** (NTAG213, URL only):

```text
http://{MOMENT_HOST}/nfc/location/{printer-slug}/{toolhead-index}
```

Phase 2 will upgrade spool tags to ICODE SLIX2 with a second NDEF record (OpenPrintTag CBOR). See Phase 2 section.
...
**Trigger:** INBXX Semi-Smart V2 ships with NFC readers in spool holder slots (targeted Q3 2026). Until then, all tag reading/writing happens via iPhone; URL-only tags are sufficient.

**Why Phase 2:** The Core One L's built-in NFC reader sits under its own spool holder. Spools in an INBXX enclosure feed via PTFE tube — the printer's reader never sees them. INBXX Semi-Smart V2 adds readers inside the enclosure slots.

### What Changes

- **Tag hardware:** NTAG213 spool tags → ICODE SLIX2 (ISO 15693 / NFC-V, longer read range)
- **Tag format:** Spool tags gain a second NDEF record:
  - Record 1: MIME type `application/vnd.openprinttag`, payload = OpenPrintTag CBOR binary
  - Record 2: URI → `http://{MOMENT_HOST}/nfc/spool/{id}` (unchanged)
- **Location tags:** remain NTAG213 URL-only
- **New dependency:** `github.com/fxamacker/cbor/v2`
- **New functions in `nfc.go`:** `SpoolToOpenPrintTag`, `EncodeOpenPrintTagCBOR`, update `BuildSpoolTagNDEF` for dual-record
...
| `materialClass` | `nfc_material_class` |
...
| `nominalLength` | `nfc_nominal_length` |
| `minPrintTemp` | `nfc_min_print_temp` |
| `maxPrintTemp` | `nfc_max_print_temp` |
| `minBedTemp` | `nfc_min_bed_temp` |
| `maxBedTemp` | `nfc_max_bed_temp` |
| `manufacturingDate` | `nfc_manufacturing_date` |
| `expirationDate` | `nfc_expiration_date` |
| `countryOfOrigin` | `nfc_country_of_origin` |
| `spoolUUID` | `nfc_spool_uuid` |
| `materialProperties` | `nfc_material_properties` (JSON array) |
